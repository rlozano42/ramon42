...
/*   Updated: 2020/07/07 12:33:00 by rlozano          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char				*ft_print(char **line, char *aux, int *ret)
{
	...
	if (aux[count] == '\n')
	{
		*line = ft_substr_gnl(aux, 0, count);
		temp = ft_strdup_gnl(aux + (count + 1));
		free(aux);
		aux = temp;
		if (aux[0] == '\0')
			aux = ft_delete(aux);
...
int						get_next_line(int fd, char **line)
{
	...
	while ((ret = read(fd, buf, BUFFER_SIZE)) > 0)
	{
		buf[ret] = '\0';
		if (aux[fd] == NULL)
			aux[fd] = ft_strdup_gnl(buf);
		else
			aux[fd] = ft_strjoin_gnl(aux[fd], buf);
